<script lang="ts">
  ...
  const libraryView = $derived<'source' | 'destination'>(
    viewParam === 'destination' ? 'destination' : 'source'
  );
  const libraryMode = $derived<LibraryPathMode>(libraryView);
  ...
  $effect(() => {
    ...
    const stored = localStorage.getItem('musichoarder-library-view') as 'grid' | 'list' | 'col' | null;
  ...
  const selectedFile = $derived(selectedFileId ? findFileById(fileSystem, selectedFileId) : null);
...
{#if !isHydrated || !isMobile.current}
  <Resizable.PaneGroup id="library-files-panels" direction="horizontal" class="min-h-0 flex-1">
