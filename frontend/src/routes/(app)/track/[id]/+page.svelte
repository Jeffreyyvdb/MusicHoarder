<script lang="ts">
  ...
  // for that same song (?song=<id>), never dead-ending the deep link. v2 mobile is
  // a later phase, so mobile users also fall back there. The flag is client-only
  // (localStorage) and the (app) group is ssr=false, so guard on mount in an effect.
  const songId = $derived(Number.parseInt(page.params.id ?? '', 10));
  const isV2 = $derived(uiVersion.isV2);
  const showTimeline = $derived(isV2 && !isMobile.current && Number.isFinite(songId));

  $effect(() => {
    if (showTimeline) return;
    // Invalid id → library; valid id but v1/mobile → the v1 provenance view.
