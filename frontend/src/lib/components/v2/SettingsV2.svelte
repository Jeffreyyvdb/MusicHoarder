<script lang="ts">
  ...
  import SharedLinksV2 from '$lib/components/v2/SharedLinksV2.svelte';
  ...
  // Soulseek and library-sync status moved to /connections, which reports every integration
  // together (including Navidrome, which had no surface at all). Settings keeps the things you
  // can actually change.
...
<div class="min-h-0 flex-1 overflow-auto pb-[var(--mh-content-pad)]">
  <div class="mx-auto flex max-w-4xl flex-col gap-6 px-4 py-4 sm:px-7 sm:py-6">
    ...
    {:else if activeTab === 'sources'}
      ...
      <!-- Integration status (Soulseek, library sync, Navidrome, Spotify) lives on
           /connections — one place for every external service. -->
      {#if user?.role === 'Owner'}
        <section class="border-border bg-card rounded-lg border px-5 py-3.5">
          <div class="flex items-center gap-4">
            <div class="min-w-0 flex-1">
              <h2 class="text-[13px] font-semibold">Connections</h2>
              <p class="text-muted-foreground text-[11.5px]">
                Spotify, Navidrome, Soulseek and library sync — status and the upgrade queue.
              </p>
            </div>
            <Button variant="outline" size="sm" href="/connections" class="shrink-0">Open</Button>
          </div>
        </section>
      {/if}
