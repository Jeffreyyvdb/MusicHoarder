<script lang="ts">
  ...
  // bare letter a stray keypress can fire. Accept's toast has an Undo, but that restores the
  // original tags and re-runs matching rather than putting the queue back as it was (undoAccept);
  // Reject has none. Skip only moves the selection, so it stays a single key. A phone has the
  // decision toolbar instead.
  //
