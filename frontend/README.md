...
This app was ported from Next.js 16 + React 19 to SvelteKit 2 + Svelte 5 (runes) + Bun. The (app) route group is rendered client-only (`ssr = false`) because the audio player and demo-mode logic read browser-only state. The landing route (`/`) keeps SSR. Six (app) pages currently ship as stubs with TODO banners pointing at the original line ranges — those page bodies (overview SSE+triggers, file browser, review queue, settings, spotify, artist detail) are the remaining porting work. The shared shell, sidebar, header, mini-player, theme toggle, demo banner, analytics, API proxy, health endpoint, player store, and full `api-client.ts` are all in place.
